...
<div class="selector" id="selector-{{ $categoryID }}">
  <span id="selector-description-{{ $categoryID }}">{{ $function['description'] }}</span>
  <ul id="selector-list-{{ $categoryID }}" class="selector_list selector_disabled">
  <li class="selector_item" onclick="{{ $function['name'] }}({{ $categoryID }}, null, '==Add to root==')">==Add to root==</li>
  @foreach ($Params->categories as $sub_category)
    ...
    @else
      <li class="selector_item" onclick="{{ $function['name'] }}({{ $categoryID }}, {{ $sub_category['id'] }}, '{{ $sub_category['name'] }}')">{{ $sub_category['name'] }}</li>
